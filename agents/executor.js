// agents/executor.js — SWARM OS v6.0
// [FIX-1] extractKeyword() kills [object Object] bug
// [FIX-2] NANA prompt forces top_pick to plain string
// [NEW-1] generate_digital_file — SVG to Supabase Storage
// [NEW-2] publish_etsy_listing — /api/etsy/publish + /api/etsy/upload-file
...
const AGENT_PROMPTS = {
  ...
  AISHA:"You are AISHA, SEO Strategist. Generate SEO for Etsy/Shopify. Return structured JSON only.",
...
const TASK_HANDLERS = {
  trend_research:async(p)=>callClaude("NANA","Research trending POD/digital products for category: \""+( p.category||"lifestyle")+"\". Return JSON: {trends:[{keyword,category,score,opportunity,tags,why_now}],top_pick:\"plain string keyword only — never an object\"}"),
  ...
  generate_etsy_title:async(p)=>{const kw=extractKeyword(p);return callClaude("AMARA","Etsy title for:\""+kw+"\". Max 140 chars, front-load keyword, end with product type. Return JSON:{title:string,confidence:0.9}");},
  generate_etsy_description:async(p)=>{const kw=extractKeyword(p);return callClaude("AMARA","Etsy description for:\""+kw+"\". 650-800 chars, hook, bullets for SVG/PNG/PDF files, usage tip. Return JSON:{description:string,confidence:0.9}");},
  generate_etsy_tags:async(p)=>{const kw=extractKeyword(p);return callClaude("AMARA","13 Etsy tags for:\""+kw+"\". Each max 20 chars, no dupes. Return JSON:{tags:[t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13],confidence:0.9}");},
  generate_social_caption:async(p)=>{const kw=extractKeyword(p);return callClaude("AMARA","TikTok/IG caption for:\""+kw+"\". Brooklyn luxury tone. Hook line 1. 3-4 lines. 3-5 hashtags. Return JSON:{caption:string,confidence:0.9}");},

  generate_digital_file:async(p)=>{
    const kw=extractKeyword(p);const tags=Array.isArray(p.tags)?p.tags:[];
    const svg=makeSVG(kw,tags);const slug=kw.toLowerCase().replace(/[^a-z0-9]+/g,"-").slice(0,60);
    const name=slug+"-"+Date.now()+".svg";
    try{
      const{error}=await supabase.storage.from("digital-products").upload(name,Buffer.from(svg,"utf-8"),{contentType:"image/svg+xml",upsert:true});
      if(error)throw new Error(error.message);
      const{data:u}=supabase.storage.from("digital-products").getPublicUrl(name);
      console.log("[executor] SVG uploaded → "+u.publicUrl);
      return{file_url:u.publicUrl,file_name:name,file_type:"svg",keyword:kw};
    }catch(uploadErr){
      console.warn("[executor] Storage upload failed: "+uploadErr.message);
...
export async function executeTask(task){
  ...
  if(task_type==="trend_research"&&result?.trends){
    for(const t of result.trends)await saveTrend({keyword:t.keyword,category:t.category||payload?.category,score:t.score!=null?parseFloat(t.score):50,source:"nana_ai",data:t});
  }
