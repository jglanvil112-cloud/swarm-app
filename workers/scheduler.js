...
const BASE_URL = (process.env.BASE_URL || process.env.RENDER_EXTERNAL_URL || "https://swarm-app-3nch.onrender.com").replace(/\/$/, "");
...
async function getSiblingOutputs(parentId) {
  if (!parentId) return {};
  const { data: siblings, error: taskError } = await supabase
    .from("tasks")
    .select("id,task_type")
    .eq("parent_task_id", parentId)
    .in("task_type", [
      "generate_etsy_title",
      "generate_etsy_description",
      "generate_etsy_tags",
      "generate_digital_file",
    ]);
  if (taskError || !siblings?.length) return {};

  const { data: outputs, error: outputError } = await supabase
    .from("agent_outputs")
    .select("task_id,output_type,etsy_title,etsy_description,etsy_tags,data")
...
async function enqueueFollowUps(task, result) {
  const parentId = task.parent_task_id;

  if (task.task_type === "trend_research" && result?.top_pick) {
    const keyword = extractTopPick(result);
    if (!keyword) return;
    const childParent = task.id;
    await Promise.all([
      enqueueTask({ agent: "AISHA", task_type: "seo_generation", payload: { keyword, title: keyword }, priority: 3, parentTaskId: childParent }),
      enqueueTask({ agent: "AMARA", task_type: "generate_etsy_title", payload: { keyword }, priority: 3, parentTaskId: childParent }),
      enqueueTask({ agent: "AMARA", task_type: "generate_etsy_description", payload: { keyword }, priority: 3, parentTaskId: childParent }),
      enqueueTask({ agent: "AMARA", task_type: "generate_etsy_tags", payload: { keyword }, priority: 3, parentTaskId: childParent }),
      enqueueTask({ agent: "AMARA", task_type: "generate_social_caption", payload: { keyword }, priority: 4, parentTaskId: childParent }),
    ]);
    return;
  }

  if (task.task_type === "generate_etsy_tags" && result?.tags && parentId) {
    const siblings = await getSiblingOutputs(parentId);
    ...
    const { count } = await supabase
      ...
      .eq("parent_task_id", parentId)
      .eq("task_type", "generate_digital_file");
    if (count) return;
  ...
  if (task.task_type === "generate_digital_file" && parentId) {
    const siblings = await getSiblingOutputs(parentId);
    const title = siblings.etsy_title?.etsy_title;
    const description = siblings.etsy_description?.etsy_description;
  ...
  if (task.task_type === "publish_etsy_listing" && result?.queued_for_approval) {
    await logAgent(
      "AISHA",
      `STAGED Etsy draft #${result.listing_id} for human approval`,
  ...
  if (task.task_type === "seo_generation" && result?.title) {
    await enqueueTask({
      agent: "AMARA",
      task_type: "social_caption",
      payload: { keyword: result.title, product: result.title, platform: "instagram" },
      priority: 4,
      parentTaskId: task.id,
    });
  }

  if (task.task_type === "analytics_report" && result?.underperformers?.length) {
    await enqueueTask({
      agent: "KWAME",
  ...
  try {
    ...
    try {
      ...
      await updateTaskStatus(
        task.id,
        "failed",
        { ...(task.result || {}), _retry_count: retryCount(task), last_error: error.message },
        error.message,
      );
      await logAgent(agent, `Failed ${task.task_type}: ${error.message}`, "error", null, task.id);
    }
  } catch (error) {
...
async function retryFailedTasks() {
  try {
    ...
    const { data: failed, error } = await supabase
      .from("tasks")
      .select("id,result,priority,updated_at")
      .eq("status", "failed")
    ...
    for (const task of failed || []) {
      ...
      if (nextRetry > MAX_RETRIES) continue;
    ...
    for (const task of stuck || []) {
      ...
      if (reclaims > 3) {
        await supabase.from("tasks").update({
          status: "failed",
          error: `stale task reclaimed ${reclaims - 1} times`,
...
async function runDailySEO() {
  const { data: trends } = await supabase.from("trends").select("keyword").order("score", { ascending: false }).limit(5);
...
async function runHealthCheck() {
  for (const service of ["anthropic", "supabase", "shopify", "etsy"]) {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/health/${service}`, { signal: AbortSignal.timeout(8000) });
  ...
  catch (error) { console.warn("[healthCheck] Etsy token check failed:", error.message); }
}

async function reslotLegacySocialPosts() {
  try {
    ...
    for (const row of rows || []) {
      ...
      await supabase
        .from("social_posts")
        .update({ scheduled_for: slot.toISOString(), updated_at: new Date().toISOString() })
        .eq("id", row.id);
      cursor = slot;
    }
...
async function runLegacyOneShotDrop() {
  if (!AUTONOMOUS_PRODUCT_DROPS) return;
  try {
    const { data: gate } = await supabase
      .from("scheduler_state")
...
// Core task, retry, staging, approval, and maintenance schedules.
cron.schedule("*/30 * * * * *", runWorkerLoop);
cron.schedule("*/5 * * * *", retryFailedTasks);
cron.schedule("*/10 * * * *", reclaimStaleTasks);
cron.schedule("*/10 * * * *", async () => {
  const result = await publishNextListing().catch(() => ({ queued: 0 }));
  if (result.queued) console.log(`[STAGE-QUEUE] queued ${result.queued}`);
});
cron.schedule("*/5 * * * *", async () => {
  const result = await drainPublishQueue(5).catch((error) => ({ error: error.message }));
...
cron.schedule("*/8 * * * *", () => { reseoTop20Tick(3).catch(() => {}); });

// Throttled Etsy maintenance.
cron.schedule("4-59/10 * * * *", () => { archiveTextOnlyTick(12).catch(() => {}); });
...
// Social operations retain their own platform approval/status logic.
cron.schedule("*/5 * * * *", () => { runAutoPublish().catch(() => {}); });
...
// Product generation is opt-in; Etsy activation still goes through the approval gate.
cron.schedule("0 5 * * *", () => runPodgenTrendDrop(0));
cron.schedule("0 9 * * *", () => runPodgenTrendDrop(1));
cron.schedule("0 12 * * *", () => runPodgenTrendDrop(2));

reslotLegacySocialPosts();
runLegacyOneShotDrop();
seedOperationalWork();
