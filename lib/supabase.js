...
export async function saveAgentOutput({ taskId, trendId, agent, outputType, etsyTitle, etsyDescription, etsyTags, socialCaption, confidence = 0 }) {
  const { data, error } = await supabase.from('agent_outputs').insert({
      task_id: taskId || null, trend_id: trendId || null, agent,
          output_type: outputType, etsy_title: etsyTitle || null,
              etsy_description: etsyDescription || null, etsy_tags: etsyTags || null,
                  social_caption: socialCaption || null,
                      confidence: typeof confidence === 'number' ? confidence : parseFloat(confidence) || 0,
                        }).select().single();
                          if (error) { console.error('[saveAgentOutput]', error.message); return null; }
                            return data;
                            }
