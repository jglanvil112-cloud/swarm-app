...
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_KEY || process.env.SUPABASE_ANON_KEY || process.env.SUPERBAS_KEY || "";
...
export async function logAgent(agent, message, level = "info", data = null, taskId = null) {
  await supabase.from("agent_logs").insert({ agent, message, level, data: data || null, task_id: taskId || null });
}

export async function saveTrend({ keyword, category, score, source, data }) {
  await supabase.from("trends").upsert({ keyword, category, score, source, data, detected_at: new Date().toISOString() }, { onConflict: "keyword" });
}

export async function saveDecision({ agent, decision_type, reasoning, data, approved = null }) {
  await supabase.from("agent_decisions").insert({ agent, decision_type, reasoning, data, approved });
}

export async function updateSchedulerState(jobName, status = "ok") {
  await supabase.from("scheduler_state").upsert({ job_name: jobName, last_run: new Date().toISOString(), last_status: status }, { onConflict: "job_name" });
}

export async function recordHealth(service, status, latencyMs = null, detail = null) {
  await supabase.from("health_checks").insert({ service, status, latency_ms: latencyMs, detail });
}
